...
from GIAOVIEN GV1
where (select count(*) from GIAOVIEN GV2 where GV2.LUONG>GV1.LUONG)=0
...
select *
from GIAOVIEN GV1
where (select count(GV2.LUONG)
					 from GIAOVIEN GV2 
					 where GV2.LUONG>GV1.LUONG and GV2.MABM=N'HTTT')=0				
--Q38. Cho biết tên giáo viên lớn tuổi nhất của bộ môn Hệ thống thông tin
select GV.HOTEN
from GIAOVIEN GV join BOMON BM on GV.MABM=BM.MABM
where BM.TENBM=N'Hệ thống thông tin' and GV.NGAYSINH <= all(select GV1.NGAYSINH
...
where K.TENKHOA=N'Công nghệ thông tin' and GV.NGAYSINH >=All (select GV1.NGAYSINH from GIAOVIEN GV1 join BOMON BM ON GV1.MABM=BM.MABM
																							JOIN KHOA K ON K.MAKHOA=BM.MAKHOA
...
from GIAOVIEN GV join BOMON BM on GV.MABM=BM.MABM
				  join KHOA K on BM.MAKHOA=K.MAKHOA
where K.TENKHOA=N'Công nghệ thông tin' and GV.MAGV not in (select GV.MAGV
					   from GIAOVIEN GV join BOMON BM on GV.MABM=BM.MABM
				                         join KHOA K on BM.MAKHOA=K.MAKHOA
...
select GV.*
from GIAOVIEN GV
where GV.MAGV not in( select GV.MAGV
...
where GV.MAGV<> GV1.MAGV and GV.PHAI=GV1.PHAI and GV.HOTEN=GV1.HOTEN
...
group by K.TENKHOA
having count(*)>=all(select count(*) from KHOA K join BOMON BM on K.MAKHOA=BM.MAKHOA
							join GIAOVIEN GV on GV.MABM=BM.MABM 
				group by K.MAKHOA)
...
having count(*)>=all(select count(*) from THAMGIADT group by MAGV)
...
where GV.MABM='HTTT'
group by GV.HOTEN, BM.TENBM
having count(*)>=all(select count(TG.MAGV) from THAMGIADT TG join GIAOVIEN GV on TG.MAGV=GV.MAGV
								     where GV.MABM='HTTT'
									 group by TG.MAGV)
...
SELECT *
FROM GIAOVIEN GV JOIN GIAOVIEN GV2 ON GV.MAGV=GV2.MAGV
WHERE GV2.MABM = 'HTTT' AND GV2.LUONG>=GV.LUONG
