...
drop procedure if exists dbo.checkPrime
create proc checkPrime
	@number int 
as
begin
	declare @i int
	declare @divisor_count int
	...
	begin
		if @number % @i = 0 
			set @divisor_count = @divisor_count + 1
		set @i=@i+1
	end
	if @divisor_count=0
...
drop procedure if exists dbo.TinhTongSoNguyenTo
create proc TinhTongSoNguyenTo
	@m int,
	@n int
as
begin
...
go
exec TinhTongSoNguyenTo 3, 5
go
--h. Tính ước chung lớn nhất của 2 số nguyên
drop procedure if exists dbo.UocChungLonNhat
create proc UocChungLonNhat
	@a int,
	@b int
as
begin
...
begin
	declare @sl int
	set @sl = 0
	select @sl = count(distinct tg.MADT)
	from THAMGIADT tg join DETAI dt on dt.MADT = tg.MADT
	where tg.MAGV = @magv and dt.NGAYKT > GETDATE()
	print N'So Luong de tai: ' + cast(@sl as varchar(10))
end
go
SLDT '004'
go
--l. In thông tin chi tiết của một giáo viên (sử dụng lệnh print): thông tin cá nhân,
----số lượng đề tài tham gia, số lượng thân nhan của giáo viên đó.
--m. Kiểm tra xem một gv có tồn tại hay ko (dựa vào MAGV)
create proc check_exists_gv
	@magv char(5)
as
begin
	if exists (select* from GIAOVIEN gv where gv.MAGV = @magv)
		print N'Co ton tai giao vien ma so: ' + @magv
	else
		print N'Khong ton tai giao vien'
end
go
check_exists_gv '001'
go
--n. Kiểm tra quy định của một gv: Chỉ được thực hiện các đề tài mà bộ môn của gv đó làm chủ nhiệm
create proc Ktra_Dtai
	@magv char(5),
	@detai char(3)
as
begin
	declare @MaBM_gv nchar(5)
	declare @MaBM_cndt nchar(5)
	select @MaBM_gv = gv.MABM from GIAOVIEN gv where @magv = gv.MAGV
	select @MaBM_cndt = gv2.MABM from GIAOVIEN gv2, DETAI dt where gv2.MAGV = dt.GVCNDT and @detai = dt.MADT
	if (@MaBM_cndt = @MaBM_gv) return 1
	return 0
end
go 
declare @c int
exec @c = Ktra_Dtai '002', '001'
print @c
go 
--o. Thực hiện thêm 1 phân công cho gv thực hiện một công việc của đề tài
----Kiểm tra thông tin đầu vào hợp lệ: gv phải tồn tại, cv phải tồn tại, thời gian tham gia phải >0
...
begin
	if not exists (select* from GIAOVIEN where MAGV = @magv)
		return 0
	if not exists (select* from CONGVIEC where (NGAYKT - NGAYBD) > 0 and @madt = MADT and @stt = SOTT)
		return 0
	declare @c int
	exec @c = Ktra_Dtai '002', '001'
	if (@c = 0)	return 0
...
go
declare @d int
exec @d = ktra_gv '002', 1, '001'
print @d
go
--p. Thực hiện xóa 1 gv theo mã. Nếu gv có thông tin liên quan (có thân nhân, có làm đề tài..) thì báo lỗi
...
begin
	declare @count int
	set @count = 0
	if exists (select* from NGUOITHAN nt where @magv = nt.MAGV)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
	if exists (select* from THAMGIADT tg where @magv = tg.MAGV)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
	if exists (select* from DETAI dt where @magv = dt.GVCNDT)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
	if exists (select* from KHOA k where @magv = k.TRUONGKHOA)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
	if exists (select* from BOMON bm where @magv = bm.TRUONGBM)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
	if exists (select* from GV_DT gvdt where @magv = gvdt.MAGV)
	begin
		raiserror(N'Lỗi...', 16, 1)
		set @count = @count + 1
	end
...
go
--q. In ra ds gv của 1 phòng ban nào đó cùng với số lượng đề tài mà gv tham gia, số thân nhân,
----số gv mà gv đó quản lý nếu có,..


--r. Kiểm tra quyu định của 2 gv a, b: Nếu a là trưởng bộ môn c của b thì lương của a phải cao hơn lương của b (a,b: mã gv)
create proc ktra_gv_a_b
	@magv_a char(5),
	@magv_b char(5)
as
begin
	declare @MaBM_a nchar(5)
	declare @MaBM_b nchar(5)

	select @MaBM_b = gv2.MABM from GIAOVIEN gv2 where gv2.MAGV = @magv_b
	if exists (select* from BOMON bm where bm.MABM = @MaBM_b and bm.TRUONGBM = @magv_a)
	begin
		if exists(select* from GIAOVIEN gv1, GIAOVIEN gv2 where gv1.MAGV = @magv_a and gv2.MAGV = @magv_b and gv1.LUONG > gv2.LUONG)
			return 1
		return 0
	end
	return 1
...
go
declare @e int
exec @e = ktra_gv_a_b '007', '008'
print @e
go
--s. Thêm 1 gv: Kiểm tra các quy định: không trùng tên, tuổi >18, lương >0
create proc add_gv @magv char(5), @ten nvarchar(40), @ngaysinh datetime,
	@luong float, @phai nchar(3), @diachi nvarchar(100), @gvql char(5), @bm nchar(5)
as
begin
	if exists (select* from GIAOVIEN gv where gv.MAGV = @magv or  gv.HOTEN = @ten and (year(@ngaysinh) - GETDATE()) > 18)
	begin
		raiserror (N'Trùng magv!', 16, 1)
		return
	end
	if exists (select* from GIAOVIEN gv where  gv.HOTEN = @ten and (year(@ngaysinh) - GETDATE()) > 18)
	begin
		raiserror (N'Trùng ten!', 16, 1)
		return
	end
	if ((year(@ngaysinh) - GETDATE()) < 18)
	begin
		raiserror (N'Thiếu tuổi!', 16, 1)
		return
	end
	if (@luong < 0)
	begin
		raiserror (N'Lương không thể bé hơn 0!', 16, 1)
		return
	end
	insert into GIAOVIEN(MAGV,HOTEN,LUONG,PHAI,NGAYSINH,DIACHI,GVQLCM,MABM)
	values (@magv, @ten, @luong, @phai, @ngaysinh, @diachi, @gvql,  @bm)
end
go
--t. Mã gv được xđ tự động theo quy tắc: Nếu đã có gv 001, 002, 003
----thì MAGV của gv mới sẽ là 004. Nếu đã có gv 001, 002, 005 thì MAGV của gv mới là 003
alter proc add_gv2  @ten nvarchar(40), @ngaysinh datetime,
	@luong float, @phai nchar(3), @diachi nvarchar(100), @gvql char(5), @bm nchar(5)
as
begin
	declare @i int
	...
	begin
		if @i<10
		begin
			set @magv = '00' + (cast(@i as varchar(3)))
			--print @magv
			if not exists (select* from GIAOVIEN where MAGV = @magv)
			begin
				insert into GIAOVIEN(MAGV,HOTEN,LUONG,PHAI,NGAYSINH,DIACHI,GVQLCM,MABM)
		...
		if @i >= 10
		begin
			set @magv = '0' + (cast(@i as varchar(3)))
			--print @magv
			if not exists (select* from GIAOVIEN where MAGV = @magv)
			begin
				insert into GIAOVIEN(MAGV,HOTEN,LUONG,PHAI,NGAYSINH,DIACHI,GVQLCM,MABM)
...
go
exec add_gv2 N'Nguyễn Thùy Linh', null, null, null,null, null, null
go